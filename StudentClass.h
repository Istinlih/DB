...
public:
		...
		for (int sem=0;sem<9;sem++)
			for (int num=0;num<10;num++)
				if (not sn->examsRecordsData[sem][num].isEmpty ){
					...
					stringList.insert(stringList.begin(), sb.setParam("examsResults_" + std::to_string(sem) + "_" + std::to_string(num) + "_s", std::to_string(sn->examsRecordsData[sem][num].special)));
			...
			if ((curSess >= 1) and (curSess <= 10))
			{
				...
				while (resultS != exitS) {
					...
					for (int i = 0; i < 10; i++)
						...
						{
							string markString = "";
							bool spec = sn->examsRecordsData[curSess - 1][i].special;
	...
	void addExamsResults(StudentNode* sn, int curSess, int curItem) {
		...
		int resultS = 1;
		const int exitS = 0;
		ClassMenu* spMenu = new ClassMenu();
		spMenu->addTitleItem("Данный предмет специальный? ");
		spMenu->addItem("Нет"); //0
		spMenu->addItem("Да"); //1
		while (resultS != exitS) {
			spMenu->run();
			resultS = spMenu->getSelectedItem();
			sn->examsRecordsData[curSess - 1][curItem].special = resultS;
			resultS = exitS;
		}
